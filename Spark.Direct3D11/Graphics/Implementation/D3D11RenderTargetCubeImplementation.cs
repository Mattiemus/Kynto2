namespace Spark.Direct3D11.Graphics.Implementation
{
    ...
    /// <summary>
    /// A Direct3D11 implementation for <see cref="RenderTargetCube"/>.
    /// </summary>
    public sealed class D3D11RenderTargetCubeImplementation : GraphicsResourceImplementation, IRenderTargetCubeImplementation, ID3D11RenderTargetView, ID3D11ShaderResourceView
    {
        private readonly D3D11RenderTargetWrapper _targetWrapper;

        internal D3D11RenderTargetCubeImplementation(D3D11RenderSystem renderSystem, int resourceID, int size, int mipMapCount, SurfaceFormat format, MSAADescription preferredMSAA, DepthFormat depthFormat, bool preferReadableDepth, RenderTargetUsage targetUsage)
            : base(renderSystem, resourceID)
        {
            _targetWrapper = new D3D11RenderTargetWrapper(renderSystem.D3DDevice, size, size, true, 1, mipMapCount, format, preferredMSAA, depthFormat, preferReadableDepth, true, targetUsage);

            Name = string.Empty;
        }

        internal D3D11RenderTargetCubeImplementation(D3D11RenderSystem renderSystem, int resourceID, SurfaceFormat format, int mipMapCount, D3D11DepthStencilBufferWrapper depthBuffer)
            : base(renderSystem, resourceID)
        {
        ...
        /// </summary>
        public int Size => _targetWrapper.Width;
        ...
        {
            return _targetWrapper.GetSubRenderTarget((int)face);
        }
        ...
        public void GetData<T>(IDataBuffer<T> data, CubeMapFace face, int mipLevel, ResourceRegion2D? subimage, int startIndex) where T : struct
        {
            ...
            ResourceHelper.ReadTextureData(resource, d3dContext, _targetWrapper.Width, _targetWrapper.Height, 1, 6, _targetWrapper.MipCount, _targetWrapper.Format, ResourceUsage.Static, data, (int)face, mipLevel, ref region, startIndex);
        }
        ...
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, CubeMapFace face, int mipLevel, ResourceRegion2D? subimage, int startIndex, DataWriteOptions writeOptions) where T : struct
        {
            ...
            ResourceHelper.WriteTextureData(resource, d3dContext, _targetWrapper.Width, _targetWrapper.Height, 1, 6, _targetWrapper.MipCount, _targetWrapper.Format, ResourceUsage.Static, data, (int)face, mipLevel, ref region, startIndex, writeOptions);
        }
