namespace Spark.Direct3D11.Graphics.Implementation
{
    ...
    /// <summary>
    /// A factory that creates Direct3D11 implementations of type <see cref="IRenderTarget2DImplementation"/>.
    /// </summary>
    public sealed class D3D11RenderTarget2DImplementationFactory : D3D11GraphicsResourceImplementationFactory, IRenderTarget2DImplementationFactory
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="D3D11RenderTarget2DImplementationFactory"/> class.
        /// </summary>
        /// <param name="renderSystem">The D3D11 render system.</param>
        public D3D11RenderTarget2DImplementationFactory(D3D11RenderSystem renderSystem)
            : base(renderSystem, typeof(RenderTarget2D))
        {
        }
        ...
        public IRenderTarget2DImplementation CreateImplementation(int width, int height, int mipMapCount, SurfaceFormat format, MSAADescription preferredMSAA, DepthFormat depthFormat, bool preferReadableDepth, RenderTargetUsage targetUsage)
        {
            return new D3D11RenderTarget2DImplementation(D3DRenderSystem, GetNextUniqueResourceId(), width, height, 1, mipMapCount, format, preferredMSAA, depthFormat, preferReadableDepth, targetUsage);
        }
        ...
        /// <returns>The render target implementation</returns>
        public IRenderTarget2DImplementation CreateImplementation(SurfaceFormat format, int mipMapCount, IDepthStencilBuffer depthBuffer)
        {
