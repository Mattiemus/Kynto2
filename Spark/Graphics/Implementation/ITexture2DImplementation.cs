...
{
    /// <summary>
    /// Defines an implementation for <see cref="Texture2D"/>.
    /// </summary>
    public interface ITexture2DImplementation : IGraphicsResourceImplementation
    {
        /// <summary>
        ...
        void GetData<T>(IDataBuffer<T> data, int mipLevel, ResourceRegion2D? subimage, int startIndex) where T : struct;
        ...
        void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int mipLevel, ResourceRegion2D? subimage, int startIndex, DataWriteOptions writeOptions) where T : struct;
    }
}
