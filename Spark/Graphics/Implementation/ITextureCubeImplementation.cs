...
{
    /// <summary>
    /// Defines an implementation for <see cref="TextureCube"/>.
    /// </summary>
    public interface ITextureCubeImplementation : IGraphicsResourceImplementation
    {
        /// <summary>
        ...
        void GetData<T>(IDataBuffer<T> data, CubeMapFace face, int mipLevel, ResourceRegion2D? subimage, int startIndex) where T : struct;
        ...
        void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, CubeMapFace face, int mipLevel, ResourceRegion2D? subimage, int startIndex, DataWriteOptions writeOptions) where T : struct;
    }
}
