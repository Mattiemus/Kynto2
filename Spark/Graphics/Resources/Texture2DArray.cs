namespace Spark.Graphics
{
    ...
    /// <summary>
    /// Represents an array of Two-Dimensional texture resources, each with a width and a height.
    /// </summary>
    public class Texture2DArray : Texture2D
    {
        ...
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, false, SurfaceFormat.Color, ResourceUsage.Static);
        }
        ...
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, ResourceUsage resourceUsage)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, false, SurfaceFormat.Color, resourceUsage);
        }
        ...
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, bool genMipMaps, SurfaceFormat format)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, genMipMaps, format, ResourceUsage.Static);
        }
        ...
        /// <param name="format">Surface format of the texture.</param>
        /// <param name="resourceUsage">Resource usage specifying the type of memory the texture should use.</param>
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, bool genMipMaps, SurfaceFormat format, ResourceUsage resourceUsage)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, genMipMaps, format, resourceUsage);
        }
        ...
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, SurfaceFormat format, params IReadOnlyDataBuffer[] data)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, false, format, ResourceUsage.Static, data);
        }
        ...
        /// <param name="data">Data to initialize the texture with, the array length must not exceed the number of array slices * number of mip map levels (in this case, each mip count is one). Data is assumed to be in the 
        /// order of the array slices, where the entire mip map chain of the first array slice comes first, then the next slice's mip map chain, and so on. Each data buffer must not exceed the size of the mip level, and is permitted to be null.</param>
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, SurfaceFormat format, ResourceUsage resourceUsage, params IReadOnlyDataBuffer[] data)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, false, format, resourceUsage, data);
        }
        ...
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, bool genMipMaps, SurfaceFormat format, params IReadOnlyDataBuffer[] data)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, genMipMaps, format, ResourceUsage.Static, data);
        }
        ...
        /// <param name="data">Data to initialize the texture with, the array length must not exceed the number of array slices * number of mip map levels. Data is assumed to be in the order of the array slices, 
        /// where the entire mip map chain of the first array slice comes first, then the next slice's mip map chain, and so on. Each data buffer must not exceed the size of the mip level, and is permitted to be null.</param>
        public Texture2DArray(IRenderSystem renderSystem, int width, int height, int arrayCount, bool genMipMaps, SurfaceFormat format, ResourceUsage resourceUsage, params IReadOnlyDataBuffer[] data)
        {
            CreateImplementation(renderSystem, width, height, arrayCount, genMipMaps, format, resourceUsage, data);
        }

        /// <summary>
        /// Gets the number of array slices in the texture. Slices may be indexed in the range [0, ArrayCount).
        /// </summary>
        public int ArrayCount => Texture2DArrayImplementation.ArrayCount;

        /// <summary>
        /// Gets the texture 2D array implementation
        /// </summary>
        private ITexture2DArrayImplementation Texture2DArrayImplementation
        {
            get => Implementation as ITexture2DArrayImplementation;
        ...
        public void GetData<T>(IDataBuffer<T> data, int arraySlice) where T : struct
        {
            ...
            try
            {
                Texture2DArrayImplementation.GetData(data, arraySlice, 0, null, 0);
            }
            catch (Exception e)
            {
        ...
        public void GetData<T>(IDataBuffer<T> data, int arraySlice, ResourceRegion2D? subimage) where T : struct
        {
            ThrowIfDisposed();

            try
            {
                Texture2DArrayImplementation.GetData(data, arraySlice, 0, subimage, 0);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error reading from resource", e);
        ...
        public void GetData<T>(IDataBuffer<T> data, int arraySlice, int mipLevel, ResourceRegion2D? subimage, int startIndex) where T : struct
        {
            ...
            try
            {
                Texture2DArrayImplementation.GetData(data, arraySlice, mipLevel, subimage, startIndex);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error reading from resource", e);
        ...
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int arraySlice) where T : struct
        {
            ...
            try
            {
                Texture2DArrayImplementation.SetData(renderContext, data, arraySlice, 0, null, 0, DataWriteOptions.Discard);
            }
            catch (Exception e)
            {
        ...
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int arraySlice, DataWriteOptions writeOptions) where T : struct
        {
            ...
            try
            {
                Texture2DArrayImplementation.SetData(renderContext, data, arraySlice, 0, null, 0, writeOptions);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error writing to resource", e);
        ...
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int arraySlice, ResourceRegion2D? subimage) where T : struct
        {
            ThrowIfDisposed();
            ThrowIfDefferedSetDataIsNotPermitted(renderContext, ResourceUsage);
            ...
            try
            {
                Texture2DArrayImplementation.SetData(renderContext, data, arraySlice, 0, subimage, 0, DataWriteOptions.Discard);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error writing to resource", e);
        ...
        /// <param name="subimage">The subimage region, in texels, of the 2D texture to write to, if null the whole image is written to.</param>
        /// <param name="writeOptions">Writing options, valid only for dynamic textures.</param>
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int arraySlice, ResourceRegion2D? subimage, DataWriteOptions writeOptions) where T : struct
        {
            ThrowIfDisposed();
            ThrowIfDefferedSetDataIsNotPermitted(renderContext, ResourceUsage);
            ...
            try
            {
                Texture2DArrayImplementation.SetData(renderContext, data, arraySlice, 0, subimage, 0, writeOptions);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error writing to resource", e);
        ...
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int arraySlice, int mipLevel, ResourceRegion2D? subimage, int startIndex) where T : struct
        {
            ThrowIfDisposed();
            ThrowIfDefferedSetDataIsNotPermitted(renderContext, ResourceUsage);
            ...
            try
            {
                Texture2DArrayImplementation.SetData(renderContext, data, arraySlice, mipLevel, subimage, startIndex, DataWriteOptions.Discard);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error writing to resource", e);
        ...
        public void SetData<T>(IRenderContext renderContext, IReadOnlyDataBuffer<T> data, int arraySlice, int mipLevel, ResourceRegion2D? subimage, int startIndex, DataWriteOptions writeOptions) where T : struct
        {
            ...
            try
            {
                Texture2DArrayImplementation.SetData(renderContext, data, arraySlice, mipLevel, subimage, startIndex, writeOptions);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error writing to resource", e);
        ...
        public IShaderResource GetSubTexture(int arraySlice)
        {
            try
            {
                return Texture2DArrayImplementation.GetSubTexture(arraySlice);
            }
            catch (Exception e)
            {
        ...
        public override void Read(ISavableReader input)
        {
            ...
            CreateImplementation(renderSystem, width, height, arrayCount, mipCount > 1, format, resourceUsage, dataArray);
        }
        ...
        {
            ITexture2DArrayImplementation impl = Texture2DArrayImplementation;
            ...
            for (int i = 0; i < arrayCount; i++)
            {
                for (int j = 0; j < mipCount; j++)
                {
                    IDataBuffer<byte> byteBuffer = DataBuffer<byte>.Create(CalculateMipLevelSizeInBytes(j, width, height, format));
        ...
        protected void ValidateCreationParameters(IGraphicsAdapter adapter, int width, int height, int arrayCount, SurfaceFormat format)
        {
        ...
        protected void ValidateCreationParameters(IGraphicsAdapter adapter, int width, int height, int arrayCount, int mipCount, SurfaceFormat format, params IReadOnlyDataBuffer[] data)
        {
            ValidateCreationParameters(adapter, width, height, arrayCount, format);
        ...
        private void CreateImplementation(IRenderSystem renderSystem, int width, int height, int arrayCount, bool genMipMaps, SurfaceFormat format, ResourceUsage resourceUsage)
        {
            ...
            ValidateCreationParameters(renderSystem.Adapter, width, height, arrayCount, format);
            
            if (!renderSystem.TryGetImplementationFactory(out ITexture2DArrayImplementationFactory factory))
            {
            ...
            try
            {
                Texture2DArrayImplementation = factory.CreateImplementation(width, height, arrayCount, mipLevels, format, resourceUsage);
            }
            catch (Exception e)
            {
        ...
        private void CreateImplementation(IRenderSystem renderSystem, int width, int height, int arrayCount, bool genMipMaps, SurfaceFormat format, ResourceUsage resourceUsage, params IReadOnlyDataBuffer[] data)
        {
            if (renderSystem == null)
            {
                throw new ArgumentNullException(nameof(renderSystem), "Render system cannot be null");
            ...
            ValidateCreationParameters(renderSystem.Adapter, width, height, arrayCount, mipLevels, format, data);
            ...
            try
            {
                Texture2DArrayImplementation = factory.CreateImplementation(width, height, arrayCount, mipLevels, format, resourceUsage, data);
            }
            catch (Exception e)
            {
                throw new SparkGraphicsException("Error while creating implementation", e);
